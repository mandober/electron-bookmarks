// https://github.com/pandanoir/List-in-JS/blob/master/src/list.js
...
export default class List extends InfinityList {
    ...
    break(f) {
        for (let i = 0, _i = this.length; i < _i; i++) {
            if (f(this.value[i]))
                return [
                    new List(this.value.slice(0, i)),
                    new List(this.value.slice(i)),
                ]
        }
    ...
    equals(b) {
        function isEquals(a, b) {
            ...
            for (let i = 0, _i = a.length; i < _i; i++) {
                if (objToString.call(a[i]) !== objToString.call(b[i]))
                    return false
    ...
    permutations() {
        ...
        for (let i = 0, _i = this.length; i < _i; i++) {
            res = res.concat(
                this.take(i)
                    .concat(this.drop(i + 1))
                    .permutations()
                    .map(l => List.of(this.value[i]).concat(l))
            )
        }
        return res
    }
    ...
    scanl(f, acc) {
        return this.foldl(
            (acc, x) => acc.concat(List.of(f(acc.last(), x))),
            List.of(acc)
        )
    }
    scanl1(f) {
        if (this.length === 0) return List.empty
        return this.tail().foldl(
            (acc, x) => acc.concat(List.of(f(acc.last(), x))),
            List.of(this.head())
        )
    }
    scanr(f, acc) {
        return this.foldr(
            (x, acc) => List.of(f(acc.head(), x)).concat(acc),
            List.of(acc)
        )
    }
    scanr1(f) {
    ...
    subsequences() {
        return this.foldl(
            (acc, x) => acc.concat(acc.map(item => item.concat(List.of(x)))),
            new List([List.empty])
        )
    }
    sum() {
    ...
    unlines() {
        let res = ""
        for (let i = 0, _i = this.length; i < _i; i++) {
            if (objToString.call(this.value[i]) !== "[object String]")
    ...
    unzip() {
        return this.unzipHelper(2)
    }
    unzip3() {
        return this.unzipHelper(3)
    }
    unzip4() {
        return this.unzipHelper(4)
    }
    unzip5() {
        return this.unzipHelper(5)
    }
    unzip6() {
        return this.unzipHelper(6)
    }
    unzip7() {
        return this.unzipHelper(7)
    }
    words() {
        const res = []
        const ws = /\s/
...
List.zipWith7 = (_f, a, b, c, d, e, f, g) =>
    List._zipWith_(_f, a, b, c, d, e, f, g)
